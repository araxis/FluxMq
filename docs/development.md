...
The `Windows Validation` workflow runs a faster Debug/no-RID/no-apphost restore/test path on pull requests. Pushes to `main`, tags, and manual workflow dispatch run release-shaped Windows restore and tests. Manual workflow dispatch also builds and uploads two artifacts:
