...
The fastest way to understand the current workspace is to open the operations dashboard and test-studio sample:

- [Download operations-dashboard-test-studio.json](/samples/operations-dashboard-test-studio.json)
- [Read the sample app walkthrough](sample-app.md)

The sample contains generated MQTT-style traffic, one pipeline, a V2 dashboard, a phase-based test scenario, and saved dashboard/test authoring metadata.
...
- Build live dashboards from reusable metric queries and dedicated MQTT operations widgets.
- Design phase-based test scenarios and run them in a separate runner console.
...
Use the Open file action to load `docs-site/public/samples/operations-dashboard-test-studio.json`.
...
- Dashboards: Design and Live modes over reusable metric queries and widget bindings.
- Tests: Scenario Designer for authoring and Runner Console for execution and investigation.
