...
The current live-tools panel polish keeps the MQTT inspector/publisher panel artifact-aware. It is available while an active pipeline or dashboard is selected, so dashboard live checks can publish test traffic without leaving the dashboard, while tests, topics, logs, and no-app states keep the full workspace width.
