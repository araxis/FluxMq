...
Current Topics direction: keep `Topics` as the app-owned tab, but make that tab behave like a full MQTT Explorer for the app's configured MQTT brokers. Opening the tab should create separate monitor clients for each app broker, subscribe those monitor clients to `#`, and keep that monitoring independent from app runtime state without adding a separate no-app explorer surface. Planned feature slices: grouped broker/topic tree with subscribe filter and optional system-topic coverage; selected-topic latest state with payload formats, metadata, retained/QoS markers, copy/export, and diff/history comparison; broker-scoped publish composer with payload modes/templates/history; lower history grid with stable pagination, filtering, and export; stats panel for broker/topic/message counts and rates. Next implementation slice: add the missing MQTT Explorer topic-detail features inside the existing `Topics` tab, starting with richer selected-topic payload/history controls.

Latest Topics idle-state/system-topic slice: when app broker monitors are connected but no retained/live messages have arrived, the `Topics` tab now shows active monitor status rows with broker endpoint, `Sub # + $SYS/#`, and connection state instead of two duplicated `No messages yet` panels. The `Topics` monitor clients use the dedicated `#,$SYS/#` subscription because MQTT `#` does not include `$SYS/...` topics; normal app/default broker subscriptions still use `#`. The history table uses a compact empty row until traffic arrives, scrolls as a virtualized full-width endless grid instead of showing a pager, scopes MudTable sizing from the local wrapper, keeps the generated table root as a real table, and uses fixed header/body cell widths for stable Time/Broker/Topic/QoS/Bytes alignment. It also has a right-side selected-message detail pane for row-level MQTT metadata/payload inspection using the same full-width label/value rows as the latest-message metadata, so selected history topics are not squeezed into half-width cells. The latest-message top panel now shares the same right detail column width as the lower selected-message details pane, with the metadata rail stretched to full height and rendered as vertical label/value rows so long topic paths are not squeezed into half-width cells. Live history retention is now per broker/topic instead of one small global buffer, so low-rate topics such as `$SYS/broker/uptime` keep growing while unrelated faster `$SYS` topics arrive; each broker/topic remains individually bounded. Next implementation slice: visually inspect Mosquitto quiet brokers and enough `$SYS` traffic to confirm the virtual history grid fills the panel, aligns correctly, and row details update smoothly, then add broker-scoped publish inside the existing `Topics` tab.

Current local design-system branch state: work is on `work/pipeline-node-ui-system`, focused on Pipeline node view/edit UI consistency rather than runtime behavior. The current accepted direction is to finish complex editor nodes first, using useful view-mode facts and stable full-height code-editor workspaces where the node authoring task is code-heavy. Dynamic Mapper, JSON Schema Validator, State Reducer, Flow Assertion, Message Filter, Condition Router, and Routing Switch have been brought onto this direction, with local commits through `Polish routing switch editor`. Flow Assertion kept runtime behavior and saved configuration unchanged, passed focused UI build/test, and visual review confirmed the left expression workspace plus compact right sidecar layout. Message Filter keeps topic-pattern validation and blank-condition behavior unchanged while moving the optional condition into a full-height code-editor workspace and the topic patterns into a compact sidecar; focused UI build/test passed, and the desktop app opened a temporary filter project though this session could not drive the WebView settings button reliably enough to complete the edit-dialog manual path. Condition Router keeps input-type normalization, type-change default expression reset, blank-condition validation, branch ports, and saved configuration unchanged while moving the route condition into a full-height code-editor workspace and sidecar controls. Routing Switch keeps expression validation, route validation, duplicate match-key validation, route parsing, ports, and saved configuration unchanged while moving the route expression into a full-height code-editor workspace and all support controls into a compact sidecar. Next implementation slice: continue remaining routing-node polish, then broaden to source/trigger, actor/sink, metric, and generic node polish.
