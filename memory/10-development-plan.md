...
The current inactive-panel polish slice makes the live MQTT inspector/publisher panel artifact-aware. It is available only while an active pipeline is selected, so dashboards, tests, logs, and no-app states keep the full workspace width and do not show misleading broker publish controls.
...
Review the current inactive-panel slice:

1. Do: open the app with no active project. Expected: the live inspector/publisher toggle is hidden, the right side panel is hidden, and the workspace uses the freed width.
2. Do: open dashboard, test, and logs artifacts. Expected: the live inspector/publisher panel remains hidden on each artifact.
3. Do: open a pipeline artifact. Expected: the live inspector/publisher toggle is visible again, and the panel can be shown or hidden.
4. Do: run `dotnet test .\tests\FluxMq.UI.Tests\FluxMq.UI.Tests.csproj --nologo` and `dotnet test .\FluxMq.sln --nologo`. Expected: all 259 UI tests and all 564 solution tests pass.

Next implementation slice: move into release readiness by tightening sample/smoke documentation and stale wording/code cleanup, while keeping any remaining designer polish as small UI-only PRs.
