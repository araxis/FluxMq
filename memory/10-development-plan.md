...
Scenario step naming rule: use the same component vocabulary across pipelines and tests where the behavior is the same. The MQTT publish action is now canonically `mqtt.publisher`, matching the normal pipeline actor; keep `mqtt.publish` only as a compatibility alias for older saved definitions such as existing `app1.json` copies.
...
- Started scenario action integration:
  ...
  - `mqtt.publish` remains a legacy saved-definition alias
...
- Fixed scenario MQTT publish ownership and saved JSON step order:
  ...
  - older `mqtt.publish` test steps are still accepted as a compatibility alias
