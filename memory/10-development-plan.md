...
Current local design-system branch state: work is on `work/pipeline-node-ui-system`, focused on Pipeline node view/edit UI consistency rather than runtime behavior. The current accepted direction is to finish complex editor nodes first, using useful view-mode facts and stable full-height code-editor workspaces where the node authoring task is code-heavy. Dynamic Mapper, JSON Schema Validator, State Reducer, and Flow Assertion have been brought onto this direction, with local commits through `Polish assertion editor`. Flow Assertion kept runtime behavior and saved configuration unchanged, passed focused UI build/test, and visual review confirmed the left expression workspace plus compact right sidecar layout. Next implementation slice: continue with Message Filter and Condition Router before broadening to source/trigger, routing, actor/sink, metric, and generic nodes.
