...
Review the current release-readiness checklist slice:

1. Do: open `docs/release-readiness.md`. Expected: local command, Windows package, manual UI, and blocker sections are present.
2. Do: follow the README/docs index links. Expected: release readiness is discoverable from both places.
3. Do: run `.\eng\verify-samples.ps1` when checking release readiness locally. Expected: the broker-free generated sample still validates and runs.

Next implementation slice: do a stale source/docs wording pass around pre-release artifacts and remove or tighten anything that still describes old right-inspector logs, pipeline-owned resources, or test-runner coupling.
