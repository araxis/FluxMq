...
Review the current release-smoke slice:

1. Do: run `.\eng\verify-samples.ps1`. Expected: the CLI builds, both sample applications validate, and `generated-traffic-inspect.json` runs for a bounded duration without a broker.
2. Do: read the README and local development doc sample sections. Expected: the broker-free generated sample is the recommended runtime smoke path.
3. Do: scan old docs snippets for the confusing `session` variable where an MQTT client is meant. Expected: MQTT publisher examples use `mqttClient`.

Next implementation slice: create a concise release-readiness checklist covering local test commands, sample verification, packaging validation, and the manual UI checks that remain before V1.
