...
Immediate smoke check: rebuild/restart the app, explicitly run the local app target, then rerun `t1` from `C:\Users\meisa\OneDrive\Documents\FluxMQ\app1.json`. Expected: step 1 is a `mqtt.publisher` scenario action that publishes `fluxmq/sample/request` from the scenario runner's separate MQTT client, step 2 matches a `MqttTrigger` `mqtt.message.received` event on `fluxmq/sample/request`, and step 3 matches a `MqttPublisher` `mqtt.message.published` event on topic `test` without needing a manual publish after the run. The scenario publisher action also emits a runner-owned `mqtt.message.published` event into the scenario journal, so broad publish expectations can match runner-owned publisher events unless filtered by topic/source. `pip2.trigger` in the sample now listens to `$SYS/#` rather than broad `#`, so `t1` is focused on the `pip1` path. UI scenario runs with `expect.event` or `when.event` require the target app runtime to be running unless the scenario itself starts runner-owned event sources such as `mqtt.publisher` or `mqtt.trigger`. The CLI `scenario` command allows `expect.event` and `when.event` after scenario-owned `mqtt.publisher` or `mqtt.trigger`, but rejects event-observing steps that appear before any runner-owned event source. `when.event` can guard later flat scenario steps: if its event filter matches, the test continues; if it times out, the guard is shown as `Skipped`, later planned steps are not run, and the scenario remains successful. Dashboard `d1` has an event counter titled `Published to test` that is configured for `mqtt.message.published`, topic prefix `test`, and status `published`; it deliberately has no QoS or retain filter, so both the pipeline's QoS 1 mapped publish and the right-inspector manual QoS 0 publish to `test` can increment it. In the Logs page, expect to see app runtime event rows such as `MqttTrigger / mqtt.message.received`, `MqttPublisher / mqtt.message.published`, and right-inspector `LivePublisher / mqtt.message.published` rows with topic/status/payload context when those events occur, scenario-owned MQTT publisher/trigger rows under `Test runner` scope, plus the scenario pass/fail summary. Isolated scenario-owned MQTT events should not increment app dashboard counters. The `t1` expectation attributes use typed JSON values (`qos: 1`, `retain: false`). The next architecture slice should keep moving toward scenario/test composition with normal components plus narrow test-specific `expect.event`/`when.event` blocks; do not add a special `mqtt.expect` side channel.
