...
Scenario runners now have a run lifetime boundary. Steps can register async cleanup for resources that must outlive a single step, and the runner reports cleanup failures as `scenario.cleanup` results. Use this for long-lived test-runner components such as a future runner-owned `mqtt.trigger`, not for ordinary short action steps such as `mqtt.publisher`.
